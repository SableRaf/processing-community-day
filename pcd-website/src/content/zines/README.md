...
`metadata.json` requires `id`, `title`, `topic`, `created_by`, `summary`, a non-empty `downloads` array, and an `intake` object containing `issue_number`, `submitted_by_github`, `submitted_date`, and `maintainer_notes`. The detail page links the submitted date to the intake issue and the submitter's username to their GitHub profile. A local download is `{ "file", "file_size", "role"? }`; `file` is a basename resolved inside `downloads/`. A manually maintained external download is `{ "url", "filename", "file_size", "role"? }`. `role`, when present, is either `reader-order` or `print-ready`. Filenames and sizes are shown to readers; there are no download labels. `cover` is optional and has `{ "src", "alt" }`, where `src` is a root-level image filename.
