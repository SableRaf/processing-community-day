...
async function showQrCode() {
  ...
  try {
    ...
    qrBlob = await new Promise<Blob>((resolve, reject) => {
      canvasRef.value?.toBlob((blob) => {
        if (blob) resolve(blob);
        else reject(new Error('Could not create a QR code image.'));
      }, 'image/png');
    });
    qrReady.value = true;
  } catch {
  ...
  <div ref="rootRef" class="share-menu" @keydown="handleRootKeydown">
    ...
    >
      <button
        type="button"
        role="menuitem"
        @click="announceCopy(markdown, t('share_menu.markdown_copied'), t('share_menu.markdown_copy_failed'))"
      >
        <svg viewBox="0 0 16 16" width="16" height="16" aria-hidden="true">
      ...
      <button
        type="button"
        role="menuitem"
        @click="announceCopy(permalink, t('share_menu.permalink_copied'), t('share_menu.permalink_copy_failed'))"
      >
        <svg viewBox="0 0 16 16" width="16" height="16" aria-hidden="true">
      ...
      <button type="button" role="menuitem" @click="showQrCode">
    ...
    >
      <div class="share-qr-dialog__header">
        ...
        <button class="share-qr-dialog__close" type="button" :aria-label="t('share_menu.close_qr')" @click="dialogRef?.close()">
          &times;
        </button>
      </div>
